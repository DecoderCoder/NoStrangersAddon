...
#include "imgui.h"
#include "blobs/file_pixelShaderBlob_blob.h"
#include "blobs/file_vertexShaderBlob_blob.h"
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
	...
	// Create the vertex shader
	{
		...
		if (bd->pd3dDevice->CreateVertexShader(file_vertexShaderBlob_blob, file_vertexShaderBlob_blob_size, NULL, &bd->pVertexShader) != S_OK)
		{
			return false;
		}
		...
		if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, file_vertexShaderBlob_blob, file_vertexShaderBlob_blob_size, &bd->pInputLayout) != S_OK)
		{
			return false;
		}
	...
	// Create the pixel shader
	{
		...
		if (bd->pd3dDevice->CreatePixelShader(file_pixelShaderBlob_blob, file_pixelShaderBlob_blob_size, NULL, &bd->pPixelShader) != S_OK)
		{
			return false;
		}
	}
...
void ImGui_ImplDX11_ReCreateFontsTexture() {
	...
	if (!bd)
		return;
	if (bd->pFontSampler) {
		bd->pFontSampler->Release(); bd->pFontSampler = NULL;
	}
