...
{
public:
	makesyscall(BYTE sysindex1, BYTE sysindex2, BYTE sysindex3, BYTE sysindex4, LPCSTR lpFuncName, uintptr_t offsetToFunc,
		BYTE retCode, BYTE ret1, BYTE ret2);
	makesyscall(BYTE sysindex1, BYTE sysindex2, BYTE sysindex3, BYTE sysindex4);
	~makesyscall();
public:
...
private:
	...
	void CreateShellSysCall(BYTE sysindex1, BYTE sysindex2, BYTE sysindex3, BYTE sysindex4, LPCSTR lpFuncName,
		uintptr_t offsetToFunc, BYTE retCode, BYTE ret1, BYTE ret2);
private:
	char* m_pShellCode;
};
...
template <typename T>
makesyscall<T>::makesyscall(BYTE sysindex1, BYTE sysindex2, BYTE sysindex3, BYTE sysindex4)
{
	CreateShellSysCall(sysindex1, sysindex2, sysindex3, sysindex4, 0, 0, 0, 0, 0);
}
...
void makesyscall<T>::CreateShellSysCall(BYTE sysindex1, BYTE sysindex2, BYTE sysindex3, BYTE sysindex4,
	LPCSTR lpFuncName, uintptr_t offsetToFunc, BYTE retCode, BYTE ret1, BYTE ret2)
{
	...
	*(BYTE*)(m_pShellCode + 7) = sysindex4;
	...
	if (!m_pShellCode)
		return;
	memcpy(m_pShellCode, ShellCode, sizeof(ShellCode));
	*(uintptr_t*)(m_pShellCode + 6) = static_cast<uintptr_t>((uintptr_t)GetProcAddress(
		(HMODULE)LGetModuleHandle("ntdll.dll"), lpFuncName) + offsetToFunc);
